...
func GenerateRPC(gen *Generator, file *File) *GenFile {
	...
	filename := path.Join(file.FilenamePrefix, file.Desc.Name+rpcFileSuffix+".ba.go")
...
func genService(g *GenFile, svc *Service) {
	// generate comment
	g.P("// ", serviceApiName, " defines RPC service for VPP binary API ", g.file.Desc.Name, ".")
	...
	g.P("conn ", g.GoIdent(govppApiPkg.Ident("Connection")))
	g.P("}")
	g.P()

	// generate client constructor
	g.P("func New", serviceClientName, "(conn ", g.GoIdent(govppApiPkg.Ident("Connection")), ") ", serviceApiName, " {")
